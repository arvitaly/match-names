export const matchNames = (name1: string, name2: string, params = { quality: 75, ignoreAbbr: false }) => {
    const result = compareNames(name1, name2, params.ignoreAbbr);
    return result.name1Quality >= params.quality || result.name2Quality >= params.quality;
};

export const compareNames = (name1: string, name2: string, ignoreAbbr: boolean) => {
    const result = {
        name1Quality: 0,
        name2Quality: 0,
    };
